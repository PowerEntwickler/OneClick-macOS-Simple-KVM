...
2. In the **EFI** volume, go into the `EFI/OC` directory and open the `config.plist` file in the macOS text editor.
...
```````````````````
 ...
 - Edit that to your preferred screen resolution, but leave the @32 bit as is.
